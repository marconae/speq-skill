...
- Local operations (`git add`, `git commit`, `git checkout -b`, `git switch`)
  run directly.
- Every operation that touches a remote — `fetch`, `push`, `gh pr create`,
  `gh pr comment`, `gh pr view`, `gh pr checkout`, `gh api` — **must** be
  proxied through `ghbrk`. Never call `git fetch`/`git push`/`gh` directly.
...
1. Classify `input`:
   - Numeric or `#N` or a PR URL → `ghbrk gh pr checkout <input>`.
   - Matches an existing local or remote branch → check it out (fetch via
     `ghbrk` first if remote-only).
   - Otherwise treat as a plan-name: check for `specs/_plans/<plan-name>/` in
     ...
     `feat/<plan-name>` exists remotely (`ghbrk git fetch`) and check it out;
...
3. `ghbrk git push` (`-u` if the branch has no upstream yet).
...
1. `ghbrk gh pr view` for the branch. If none exists, `ghbrk gh pr create`
   with the given title/body/draft flag against the default branch.
2. If a PR already exists, do not recreate it — the push in
   `commit-and-push` already updated it. If `draft` is `false` and the
   existing PR is a draft, mark it ready (`ghbrk gh pr ready`).
...
5. `ghbrk gh pr comment` with the same checklist from step 1.
...
1. `ghbrk gh pr view --json comments,reviews` (or equivalent `gh api`) for the
   branch's PR.
...
3. `ghbrk gh pr ready` if the PR is currently a draft.
...
- Never call `git fetch`, `git push`, or any `gh` subcommand directly —
  always through `ghbrk`.
...
| Pattern | Why Wrong |
|---------|-----------|
| Calling `git push`/`gh` directly | Bypasses the `ghbrk` remote policy |
