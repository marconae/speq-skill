...
if [[ "$VERSION_OUTPUT" != "speq 0.8.0" ]]; then
    echo "FAIL: expected speq 0.8.0 after first install, got: $VERSION_OUTPUT"
    exit 1
fi
...
if [[ "$VERSION_OUTPUT" != "speq 0.8.0" ]]; then
    echo "FAIL: expected speq 0.8.0 after update, got: $VERSION_OUTPUT"
    exit 1
fi
