...
if [[ "$VERSION_OUTPUT" != "speq 0.5.1" ]]; then
    echo "FAIL: expected speq 0.5.1 after first install, got: $VERSION_OUTPUT"
    exit 1
fi
...
if [[ "$VERSION_OUTPUT" != "speq 0.5.1" ]]; then
    echo "FAIL: expected speq 0.5.1 after update, got: $VERSION_OUTPUT"
    exit 1
fi
